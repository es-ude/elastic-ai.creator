{
  inputs = let
    ...
    devenv_root = "/Users/lukas/Projects/ies-ude/elasticai-new-creator";
    devenv_dotfile = ./.devenv;
    devenv_dotfile_string = ".devenv";
    container_name = null;
    devenv_tmpdir = "/var/folders/9h/f9hly_c55vz4sdg1xjzx6z_00000gn/T/";
    devenv_runtime = "/var/folders/9h/f9hly_c55vz4sdg1xjzx6z_00000gn/T/devenv-4ab2af5";
  ...
  outputs = {nixpkgs, ...} @ inputs: let
    ...
    project = pkgs.lib.evalModules {
      ...
      modules =
        ...
        ++ [
          (
            if builtins.pathExists ./devenv.nix
            then ./devenv.nix
            else {}
          )
          (devenv.devenv or {})
          (
            if builtins.pathExists ./devenv.local.nix
            then ./devenv.local.nix
            else {}
          )
          (
            if builtins.pathExists (devenv_dotfile + "/cli-options.nix")
            then import (devenv_dotfile + "/cli-options.nix")
            else {}
          )
        ];
    };
    ...
    options = pkgs.nixosOptionsDoc {
      ...
      transformOptions = let
        isDocType = v: builtins.elem v ["literalDocBook" "literalExpression" "literalMD" "mdDoc"];
      in
        lib.attrsets.mapAttrs (
          _: v:
            if v ? _type && isDocType v._type
            then v.text
            else if v ? _type && v._type == "derivation"
            then v.name
            else v
        );
    };
    ...
    build = options: config:
      ...
      (
        name: option:
          ...
          in
            if builtins.elem typeName ["output" "outputOf"]
