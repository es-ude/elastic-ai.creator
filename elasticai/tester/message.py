...
class Message:
    ...
    def __repr__(self):
        return (
            f"Message(command={self.command.name}, payload={'...'.join([str(self.payload[:10]), str(self.payload[-10:])]) if self.payload_size > 20 else self.payload}, checksum={self.checksum},"
