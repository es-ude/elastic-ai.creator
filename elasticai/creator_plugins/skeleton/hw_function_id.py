...
class HwFunctionIdUpdater:
    def __init__(
        ...
        target_file_name = "skeleton_pkg.vhd"
        target_file = None
        for top, dirs, files in os.walk(build_dir):
            if target_file_name in files:
                target_file = Path(top) / target_file_name
        if target_file is None:
            raise FileNotFoundError(
                f"target file {target_file_name} not found in build dir {build_dir}"
            )
