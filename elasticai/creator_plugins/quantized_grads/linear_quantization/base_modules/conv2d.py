...
class Conv2d(LinearQuantizationLayer, TorchConv2d):
    def __init__(
        ...
        if not isinstance(weight_quantization, ParamQuantizationModule):
            raise TypeError(f"{weight_quantization} should be a {ParamQuantizationModule.__class__.__name__}")
        if not isinstance(bias_quantization, ParamQuantizationSimulatedModule):
            raise TypeError(f"{bias_quantization} should be a {ParamQuantizationSimulatedModule.__class__.__name__}")

        if bias ^ isinstance(bias_quantization, ParamQuantizationSimulatedModule):
            raise Exception(
                f"if bias is True, bias_quantization needs to be set."
