import pytest
import torch
from torch.nn import Sequential, Linear, ReLU, Module, Conv2d, BatchNorm2d, Flatten
...
from elasticai.creator_plugins.ir_shape_inference.shape_inference import IrShapeInference
from elasticai.creator_plugins.ir_shape_inference.default_handlers import (
    linear as linear_shape,
    relu as relu_shape,
    conv1d as conv1d_shape,
...
def test_ir2shapes_skip_connection():
    ...
    y = m(torch.ones(shape))
    print(y.shape)
    root, reg = convert2ir(m)
    translator = shape_translator()
    new_graph = translator(root, reg, {"x": shape})
    print(serialize(new_graph, reg))
    assert serialize(new_graph, reg) == {
        "conv2d": {
            "type": "conv2d",
            "in_channels": 2,
