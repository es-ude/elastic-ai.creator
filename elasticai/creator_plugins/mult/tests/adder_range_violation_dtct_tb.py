...
def dtct_model(a: int, bitwidth: int, num_adders: int, signed: bool) -> tuple[int, int, int]:
    valrange = [0, 2**(bitwidth-num_adders)-1] if not signed else [-2**(bitwidth-1-num_adders), 2**(bitwidth-1-num_adders)-1]
    down = a < valrange[0]
    upper = a > valrange[1]
...
async def violation_chck(dut):
    valrange = [0, 2**(dut.BITWIDTH.value.to_unsigned())-1] if not dut.SIGNED.value else [-2**(dut.BITWIDTH.value.to_unsigned()-1), 2**(dut.BITWIDTH.value.to_unsigned()-1)-1]
    for idx in range(valrange[0], valrange[1]):
        dut.A.value = idx

        await Timer(2, unit='step')
        assert (dut.DOWNER_LIMIT.value, dut.UPPER_LIMIT.value, dut.DATA_VALID.value) == dtct_model(idx, dut.BITWIDTH.value.to_unsigned(), dut.NUM_ADDERS.value.to_unsigned(), dut.SIGNED.value)
