...
from .node_protocol import _NodeP

P = ParamSpec("P")


def _type_handler_fn(
    name: str, fn: Callable[[Graph], Implementation]
) -> _pl.PluginSymbol:
    def load_into(lower: LoweringPass[Graph, Implementation]):
...
def _iterable_type_handler_fn(
    name: str, fn: Callable[[Graph], Iterable[Implementation]]
) -> _pl.PluginSymbol:
    def load_into(lower: LoweringPass[Graph, Iterable[Implementation]]):
...
class _FilterNode(Node):
    params: RequiredField[dict, FilterParameters] = RequiredField(
        set_convert=lambda x: x.to_dict(), get_convert=FilterParameters.from_dict
    )
...
def append_counter_suffix_before_construction(
    ...
    def construct(*args: P.args, **kwargs: P.kwargs) -> Node:
        ...
        if "name" in kwargs:
            ...
            count = counters.get(name, 0)
            counters[name] = count + 1
            node = fn(name, *args[1:], **kwargs)
            return node

    return construct


class _Sequential(Implementation):
    def __init__(self, name: str):
        super().__init__(name, type="clocked_combinatorial", attributes={})
    ...
    def add_input(self, shape: Shape):
        ...
        self._last_node = self.nodes["input"]

    def filter(self, n: _NodeP):
        node = _FilterNode(n.data)
        attributes = n.attributes
        params = node.params
        if "top_stride" not in self.attributes:
            self.attributes.update(
                {
                    "top_stride": params.in_channels * params.stride,
                }
            )
        elif "params" in self._last_node.data:
    ...
    def _append_static(
        self, name: str, implementation: str, output_shape: tuple[int, int], **kwargs
    ):
        self._append_node(
            name=name,
    ...
    def shift_register(self, name: str, output_shape: tuple[int, int]):
        self._append_static(
            name=name,
            output_shape=output_shape,
    ...
    def input(self, shape: Shape, impl: Implementation) -> None:
        input_shape = self._determine_required_input_shape(impl)
        self.attributes["top_kernel_size"] = input_shape.size()
...
@_type_handler
def sequential(impl: Graph) -> Implementation:
    seq = _Sequential(impl.name)

    def iter_nodes():
        for name in dfs_pre_order(impl.successors, "input"):
            yield impl.nodes[name]
    ...
    for n in iter_nodes():
        match n.type:
            case "filter":
                seq.filter(n)
            case "input":
                seq.attributes["runtime_input_shape"] = n.input_shape.to_tuple()
                seq.input(n.input_shape, impl)
            case "output":
                seq.attributes["runtime_output_shape"] = n.output_shape.to_tuple()
                seq.finish()
            case _:
                raise Exception(f"Unknown type {n.type}")

    return seq


@_iterable_type_handler
def network(impl: Graph) -> Iterable[Implementation]:
    ...
    skeleton: Implementation[Node, Edge] = Implementation(
        name="skeleton", type="skeleton", attributes={}
    )
    skeleton.attributes["generic_map"] = {
    ...
    buffered_network_wrapper: Implementation[Node, Edge] = Implementation(
        name="buffered_network_wrapper",
        type="buffered_network_wrapper",
        attributes={},
    )
    buffered_network_wrapper.attributes["generic_map"] = {
