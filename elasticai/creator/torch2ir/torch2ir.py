...
class Torch2Ir:
    ...
    def _check_handler_and_get_name(
        self, name: str | None, handler: Callable[[Module], dict]
    ) -> str:
        if name is None:
            if hasattr(handler, "__name__") and isinstance(handler.__name__, str):
                return handler.__name__
            else:
                raise TypeError("specify type explicitly if handler is not a function")
        return name
    ...
    def register_handlers(
        ...
        for handler in handlers:
            self.register(self._check_handler_and_get_name(None, handler), handler)
        return self
