...
class SymbolFetcherBuilder(Generic[PluginSpecT, _PlRecT]):
    ...
    * `spec_type`: The type of the plugin spec that the `SymbolFetcher` will build from the plugin dictionaries.
    ...
    .Example
    ```python
    from elasticai.creator.plugin import SymbolFetcherBuilder, PluginSpec, PluginLoader
    from typing import TypeVar

    T = TypeVar("T", bound="MyPluginLoader")

    class _ExtraSymbol(PluginSymbol[T]):
      def load_into(self, receiver: T) -> None:
        receiver.print_symbol("extra")

    class NamePrintingLoader(PluginLoader):
      ...
      def print_symbol(self, name: str) -> None:
        print(name)
    ```
