...
class SoftmaxLUT(DesignCreatorModule, nn.Module):
    ...
    def create_design(self, name: str) -> SoftmaxLUTDesign:
        ...
        return SoftmaxLUTDesign(
            ...
            dim_b=self.dim_b,
            dim_c=self.dim_c,
            Qinput2QDenominator_LUT_dict=self.Qinput2QDenominator_LUT_dict,
            Qinput2QNumerator_LUT_dict=self.Qinput2QNumerator_LUT_dict,
    ...
    def precompute(self):
        ...
        if self.quant_bits <= 4:
            ...
            self.z_t = z_t.round_().clamp(
                -(2 ** (self.quant_bits * 3 - 1)), 2 ** (self.quant_bits * 3 - 1) - 1
            )

            q_t = t / s_t + self.z_t
            q_t = q_t.to(dtype=torch.int32)
            q_t = q_t.round_().clamp(
                -(2 ** (self.quant_bits * 3 - 1)), 2 ** (self.quant_bits * 3 - 1) - 1
            )
        else:
            s_t = (1.0 - 0.0) / (
                (
                    (2 ** (self.quant_bits * 2 - 1) - 1)
                    - (-(2 ** (self.quant_bits * 2 - 1)))
                )
                // ((self.window_size**2) * self.nhead)
            )
            z_t = (2 ** (self.quant_bits * 2 - 1) - 1) - (
                1.0 / s_t
            )  # get zero point of t
            z_t = torch.tensor(z_t)
            self.z_t = z_t.round_().clamp(
                -(2 ** (self.quant_bits * 2 - 1)), 2 ** (self.quant_bits * 2 - 1) - 1
            )
    ...
    def forward(
        ...
        self.inputs1_QParams = given_inputs_QParams  # QParams before doing subtraction
