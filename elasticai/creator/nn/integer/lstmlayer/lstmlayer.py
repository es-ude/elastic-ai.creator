...
from elasticai.creator.nn.integer.quant_utils.lstmcell import LSTMCell
from elasticai.creator.nn.integer.quant_utils.lstmlayer.design import (
    LSTMLayer as LSTMLayerDesign,
)
...
class LSTMLayer(nn.Module):
    ...
    def int_forward(
        self,
        q_inputs: torch.FloatTensor,
        q_h_prev: torch.FloatTensor,
        q_c_prev: torch.FloatTensor,
    ) -> torch.FloatTensor:
        ...
        for t in range(self.seq_len):
            ...
            q_hidden_states.append(q_h_next.unsqueeze(1))

        q_outputs = torch.stack(q_hidden_states, dim=1)
