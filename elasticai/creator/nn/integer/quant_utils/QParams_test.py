...
class TestQParams(unittest.TestCase):
    def test_AsymmetricSignedQParams(self):
        ASSSParams = AsymmetricSignedQParams(
        ...
        self.assertTrue(torch.equal(q_x, expected_q_x))

        dq_x = ASSSParams.dequantize(q_x)
        expected_dq_x = torch.tensor(
            [[-2.0000, 0.5098, 1.0000, 2.0000, 3.0000]], dtype=torch.float32
        )
        self.assertTrue(torch.allclose(dq_x, expected_dq_x, atol=1e-4))

    def test_AsymmetricUnsignedQParams(self):
    ...
    def test_SymmetricSignedQParams(self):
        SSSSParams = SymmetricSignedQParams(
            quant_bits=8, observer=GlobalMinMaxObserver()
        )
    ...
    def test_SymmetricUnsignedQParams(self):
        # BUG:  this scheme lead to huge quantization error
        SSUSParams = SymmetricUnsignedQParams(
            quant_bits=8, observer=GlobalMinMaxObserver()
        )
    ...
    def test_quant_range(self):
        ...
        self.assertTrue(torch.equal(SSSSParams.min_quant, expected_min_quant))
        self.assertTrue(torch.equal(SSSSParams.max_quant, expected_max_quant))
    ...
    def test_out_of_range_values(self):
        ...
        q_x = ASSSParams.quantize(x)
        self.assertTrue(torch.all(q_x >= ASSSParams.min_quant.item()))
        self.assertTrue(torch.all(q_x <= ASSSParams.max_quant.item()))
    ...
    def test_state_dict(self):
        ...
        self.assertTrue(torch.equal(ASSSParams.min_float, ASSSParams_new.min_float))
        self.assertTrue(torch.equal(ASSSParams.max_float, ASSSParams_new.max_float))
        self.assertTrue(
            torch.equal(ASSSParams.scale_factor, ASSSParams_new.scale_factor)
        )
        self.assertTrue(torch.equal(ASSSParams.zero_point, ASSSParams_new.zero_point))
