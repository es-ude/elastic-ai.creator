...
class SepResidualBlock(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.sepconv1dbn_0 = SepConv1dBN(
            in_channels=in_channels,
            out_channels=out_channels,
            kernel_size=kernel_size,
            seq_len=seq_len,
            padding=1,
            name=self.name + "_sepconv1dbn_0",
            quant_bits=self.quant_bits,
            quant_data_dir=self.quant_data_dir,
            device=device,
        )
        self.sepconv1dbn_0_relu = ReLU(
            name=self.name + "_sepconv1dbn_0_relu",
            quant_bits=self.quant_bits,
            quant_data_dir=self.quant_data_dir,
            device=device,
        )
        self.sepconv1dbn_1 = SepConv1dBN(
            in_channels=out_channels,
            out_channels=out_channels,
            kernel_size=kernel_size,
            seq_len=seq_len,
            padding=1,
            name=self.name + "_sepconv1dbn_1",
            quant_bits=self.quant_bits,
            quant_data_dir=self.quant_data_dir,
            device=device,
        )
    ...
    def create_design(self, name: str) -> SepResidualBlockDesign:
        return SepResidualBlockDesign(
            ...
            sepconv1dbn_0=self.sepconv1dbn_0,
            sepconv1dbn_0_relu=self.sepconv1dbn_0_relu,
            sepconv1dbn_1=self.sepconv1dbn_1,
    ...
    def precompute(self) -> None:
        ...
        self.sepconv1dbn_0.precompute()
        self.sepconv1dbn_1.precompute()
    ...
    def int_forward(self, q_inputs: torch.IntTensor) -> torch.IntTensor:
        ...
        q_outputs = self.sepconv1dbn_0.int_forward(q_inputs)
        q_outputs = self.sepconv1dbn_0_relu.int_forward(q_outputs)
        q_outputs = self.sepconv1dbn_1.int_forward(q_outputs)
    ...
    def forward(
        ...
        residual = inputs
        outputs = self.sepconv1dbn_0.forward(
            inputs=inputs,
            given_inputs_QParams=self.inputs_QParams,
            enable_simquant=enable_simquant,
        )

        outputs = self.sepconv1dbn_0_relu.forward(
            inputs=outputs,
            given_inputs_QParams=self.sepconv1dbn_0.outputs_QParams,
            enable_simquant=enable_simquant,
        )

        outputs = self.sepconv1dbn_1.forward(
            inputs=outputs,
            given_inputs_QParams=self.sepconv1dbn_0_relu.outputs_QParams,
            enable_simquant=enable_simquant,
        )
        ...
        add_outputs = self.add.forward(
            ...
            given_inputs2_QParams=self.sepconv1dbn_1.outputs_QParams,
            enable_simquant=enable_simquant,
        )
