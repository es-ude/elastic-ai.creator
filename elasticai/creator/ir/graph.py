...
class AdjacencyMap[K, V](Mapping[K, Mapping[K, V]]):
    ...
    def __repr__(self) -> str:
        return f"NestedMapping({repr(self._mapping)})"
...
class GraphImpl[T: Hashable, E](Graph[T, E]):
    ...
    def __eq__(self, other: object) -> bool:
        if hasattr(other, "successors"):
            return self.successors == other.successors  # type: ignore[attr-defined]
        return False
    ...
    def remove_edge(self, src: T, dst: T, /) -> Self:
        new_predecessors = self.predecessors.drop(src, dst)
        new_successors = self.successors.drop(dst, src)
        return type(self)(
            self._default_edge_attributes_factory,
            predecessors=new_predecessors,
            successors=new_successors,
