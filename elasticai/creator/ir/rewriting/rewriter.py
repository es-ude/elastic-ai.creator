...
from elasticai.creator.graph import Graph, find_all_subgraphs, rewrite
...
class RemappedSubImplementation(Generic[N]):
    ...
    def predecessors(self, node: str) -> Mapping[str, N]:
        return self._create_remapped_data(self._impl.predecessors(node))
...
class _RemappedData(Mapping[str, N]):
    def __init__(
        ...
        node_fn: Callable[[str, dict[str, Any]], N] | None = Node,
    ) -> None:
...
class Rewriter:
    ...
    def _lift_constraint_fn(
        ...
        def lifted(pattern_node: str, original_node: str) -> bool:
            return fn(pattern.nodes[pattern_node], impl.nodes[original_node])
