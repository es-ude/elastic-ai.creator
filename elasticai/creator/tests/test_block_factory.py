import unittest
from elasticai.creator.block_factory import generate_conv2d_sequence_with_width
import torch
...
class BlockFactoryTest(unittest.TestCase):
    ...
    def test_generate_conv2d_sequence_with_width_base(self):
        ...
        expected = torch.nn.Sequential(QConv2d_block(in_channels=2,out_channels=4,activation=torch.nn.Identity(),conv_quantizer=Binarize(),kernel_size=3),ChannelShuffle(groups=1))
    ...
    def test_generate_conv2d_sequence_with_width_more_complex(self):
        ...
        expected = torch.nn.Sequential(QConv2d_block(in_channels=4,out_channels=4,activation=torch.nn.Identity(),conv_quantizer=Binarize(),kernel_size=3,groups=2),ChannelShuffle(groups=2),QConv2d_block(in_channels=4,out_channels=2,activation=torch.nn.Identity(),conv_quantizer=Binarize(),kernel_size=3,groups=2))
        self.compare_models_and_weight_shape(layers,expected)
    ...
    def test_generate_conv2d_sequence_with_width_more_than_2_last_groups_unequal_out_channels(self):
        ...
        expected = torch.nn.Sequential(QConv2d_block(in_channels=256,out_channels=256*32,activation=torch.nn.Identity(),conv_quantizer=Binarize(),kernel_size=3,groups=32),ChannelShuffle(groups=32),QConv2d_block(in_channels=256*32,out_channels=256*4,activation=torch.nn.Identity(),conv_quantizer=Binarize(),kernel_size=3,groups=256*4),QConv2d_block(in_channels=256*4,out_channels=256,activation=torch.nn.Identity(),conv_quantizer=Binarize(),kernel_size=3,groups=128))
        self.compare_models_and_weight_shape(layers,expected)
    ...
    def test_generate_conv2d_sequence_with_width_more_than_2_last_groups_equal_out_channels(self):
        ...
        expected = torch.nn.Sequential(QConv2d_block(in_channels=64,out_channels=256*8,activation=torch.nn.Identity(),conv_quantizer=Binarize(),kernel_size=3,groups=8),ChannelShuffle(groups=8),QConv2d_block(in_channels=256*8,out_channels=256,activation=torch.nn.Identity(),conv_quantizer=Binarize(),kernel_size=3,groups=256))
        self.compare_models_and_weight_shape(layers,expected)
