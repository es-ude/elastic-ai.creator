import unittest
from elasticai.creator.iotable import IOTable,group_tables
import  numpy as np
...
class test_Io_table_builder(unittest.TestCase):
    def test_group_tables_basic(self):
        ...
        self.assertTrue(np.all(inputs==result[0].tables[0]),np.all(outputs==result[0].tables[1]))
    ...
    def test_create_io_dict_depthwise(self) -> None:
        io_list =  [IOTable(np.asarray([[1,2],[3,4]]),np.asarray([[1], [6]])),IOTable(np.asarray([[3,4],[5,6]]),np.asarray([[2], [5]]))] 
