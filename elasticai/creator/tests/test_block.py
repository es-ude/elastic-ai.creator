...
from elasticai.creator.blocks import QConv1d_block, depthwiseQConv1d_block, QLinear_block


class BlockTests(unittest.TestCase):
    def test_conv1d_block(self):
        with self.subTest("test params"):
            module = QConv1d_block(in_channels=2, out_channels=32, kernel_size=3, conv_quantizer=Binarize(),
                                   activation=Ternarize(), stride=1,
                                   padding=0,
                                   dilation=1,
                                   groups=2,
                                   bias=True,
        ...
        with self.subTest("test call"):
            module = QConv1d_block(in_channels=1, out_channels=32, kernel_size=3, conv_quantizer=Binarize(),
                                   activation=Ternarize(), stride=1,
                                   padding=0,
                                   dilation=1,
                                   groups=1,
    ...
    def test_Depthwiseconv1d_block(self):
        with self.subTest("test params"):
            module = depthwiseQConv1d_block(in_channels=4, out_channels=32, kernel_size=3, conv_quantizer=Binarize(),
                                            activation=Ternarize(), stride=1,
                                            padding=0,
                                            dilation=1,
                                            bias=True,
        ...
        with self.subTest("test call"):
            module = depthwiseQConv1d_block(in_channels=2, out_channels=32, kernel_size=3, conv_quantizer=Binarize(),
                                            activation=Ternarize(), stride=1,
                                            padding=0,
                                            dilation=1,
                                            bias=True,
    ...
    def test_Linear_block(self):
        with self.subTest("test params"):
            module = QLinear_block(in_features=16, out_features=4, linear_quantizer=Binarize(),
