...
class Loader(_Loader[Implementation, Code]):
    """A plugin loader for vhdl plugins."""

    def _get_loadables(self, plugin_spec: PluginSpec) -> dict[str, str]:
        if plugin_spec.target_runtime in ("vhdl",):
            return super()._get_loadables(plugin_spec)
