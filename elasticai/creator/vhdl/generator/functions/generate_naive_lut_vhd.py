...
def main(example_inputs, example_outputs,file_path):
    ...
    with open(file_path, "w") as writer:
        ...
        LUT = NaiveLUTBasedConv(component_name="NaiveLUTBasedConv_rtl",inputs=encoded_inputs,outputs=encoded_outputs,input_width=input_logic_encoder.bit_width,output_width=output_logic_encoder.bit_width)
        for line in LUT():
            writer.write(line)
            if line[-1]!= "\n":
                writer.write("\n")
