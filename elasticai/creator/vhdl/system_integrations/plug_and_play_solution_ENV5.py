...
class FirmwareENv5:
    ...
    def save_to(self, destination: Path):
        def save_srcs(destination: Path):
            ...
            lstm_skeleton.save_to(
                destination.create_subpath(subpath_name=self._network.name)
            )
            ...
            env5_reconfig_top.save_to(
                destination.create_subpath(subpath_name="env5_reconfig_top")
            )
