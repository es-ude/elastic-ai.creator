...
class SinusDataset(Dataset):
    def __init__(self, sinus_range=(1, 500), num_points=5000, window_size=100):
        ...
        self._full_series = torch.sin(x) * (self.sinus_range[1] - x) / self.sinus_range[1]
        self._data = self._rolling_samples_with_labels(self._full_series, self.window_size)
...
if __name__ == "__main__":
    ...
    for epoch in range(n_epochs):
        ...
        with torch.no_grad():
            val_loss = sum(loss_fn(lstm_model(X).view(-1, ), y) for X, y in dl_test) / len(dl_test)

        print("Epoch: {}/{}; Loss: {}; Val_Loss: {}".format(epoch + 1, n_epochs, train_loss, val_loss))
    ...
    for i in range(ws):
        ...
        predicted = predicted[1:] + [y_pred.view(-1, ).tolist()]
    x_axis = torch.arange(0, ws * 2)
    plt.plot(x_axis[:ws], torch.tensor(sample).view(-1, ), "-b", label="previous window")
    plt.plot(x_axis[ws:], torch.tensor(target).view(-1, ), "-g", label="target prediction")
    plt.plot(x_axis[ws:], torch.tensor(predicted).view(-1, ), "-r", label="actual prediction")
    plt.legend()
    plt.show()
