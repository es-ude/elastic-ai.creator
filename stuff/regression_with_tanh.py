...
from elasticai.creator.nn.vhdl import FPHardTanh, FPLinear, Sequential
...
class Model(torch.nn.Module):
    def __init__(self, total_bits=16, frac_bits=8) -> None:
        ...
        # self.tanh = torch.nn.Sequential(torch.nn.Tanh(), self.quantize)
        self.tanh = FPHardTanh(
            total_bits=total_bits,
            frac_bits=frac_bits,
            # num_steps=128,
        ...
        self.model = Sequential(self.lin1, self.tanh, self.lin2)

    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        return self.model(self.quantize(inputs))
