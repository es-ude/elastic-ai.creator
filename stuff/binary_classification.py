...
def get_dataset(samples_per_cluster: int = 500) -> TensorDataset:
    xs_c0 = torch.empty(samples_per_cluster, 1).normal_(mean=0, std=6)
    ys_c0 = torch.empty(samples_per_cluster, 1).normal_(mean=2, std=0.2)
    samples_c0 = torch.cat([xs_c0, ys_c0], dim=1)
    labels_c0 = torch.ones(samples_per_cluster) * 0

    xs_c1 = torch.empty(samples_per_cluster, 1).normal_(mean=0, std=6)
    ys_c1 = torch.empty(samples_per_cluster, 1).normal_(mean=0, std=0.2)
    samples_c1 = torch.cat([xs_c1, ys_c1], dim=1)
    labels_c1 = torch.ones(samples_per_cluster) * 1

    samples = torch.cat([samples_c0, samples_c1])
    labels = torch.cat([labels_c0, labels_c1]).to(torch.float32)
