...
class Splittable:
    ...
    def split_by_group(
        ...
        return split_by_group(self._fp, max_fan_in=max_fan_in, min_fan_in=min_fan_in)
...
def split_by_group(
    ...
    for split in result:
        if (
            min_fan_out <= split[0].out_channels <= max(f.out_channels, f.in_channels)
...
def split_by_group_end_with_pointwise(
    ...
    return _split_by_groups_with_kernel_sizes(
        original, max_fan_in, (original.kernel_size, 1), (original.stride, 1)
    )
...
def split_by_group_start_with_pointwise(
    ...
    return _split_by_groups_with_kernel_sizes(
        original, max_fan_in, (1, original.kernel_size), (1, original.stride)
    )


def _split_by_groups_with_kernel_sizes(
    original: FilterParameters,
    max_fan_in: int,
