...
module MULT_SIGNED#(
    ...
    //################################### Step 2: Accumulation ###################################
    //Row 2
    ADDER_HALF PP_row10(IN[0][1], IN[1][0],   CC[0][0],   PP[0][0]);
    for(x0=1; x0 < (BITWIDTH-'d1); x0 = x0+1) begin
        ADDER_FULL PP_row11(IN[0][x0+'d1], IN[1][x0], CC[0][x0-'d1], CC[0][x0], PP[0][x0]);
    end
    ADDER_FULL PP_row13(1'b1, IN[1][BITWIDTH-'d1],  CC[0][BITWIDTH-'d2],  CC[0][BITWIDTH-'d1],  PP[0][BITWIDTH-'d1]);
    assign Q[1:0] = {PP[0][0], IN[0][0]};
    
    //Row 3 to BITWIDTH-1
    for(y0 = 2; y0 < (BITWIDTH-'d1); y0 = y0 +'d1) begin
        ADDER_HALF PP_row20(PP[y0-'d2][1], IN[y0][0],   CC[y0-'d1][0],   PP[y0-'d1][0]);
        for(x0=1; x0 < (BITWIDTH-'d1); x0 = x0+1) begin
            ADDER_FULL PP_row21(PP[y0-'d2][x0+'d1], IN[y0][x0], CC[y0-'d1][x0-'d1], CC[y0-'d1][x0], PP[y0-'d1][x0]);
        end
        ADDER_FULL PP_row22(CC[y0-'d2][BITWIDTH-'d1], IN[y0][BITWIDTH-'d1],  CC[y0-'d1][BITWIDTH-'d2],  CC[y0-'d1][BITWIDTH-'d1],  PP[y0-'d1][BITWIDTH-'d1]);
        assign Q[y0] = PP[y0-'d1][0];
    end 
    //Last Row    
    ADDER_HALF PP_row20(PP[BITWIDTH-'d3][1], IN[BITWIDTH-'d1][0],   CC[BITWIDTH-'d2][0],   PP[BITWIDTH-'d2][0]);
    for(x0=1; x0 < (BITWIDTH-'d1); x0 = x0+1) begin
        ADDER_FULL PP_row21(PP[BITWIDTH-'d3][x0+'d1], IN[BITWIDTH-'d1][x0], CC[BITWIDTH-'d2][x0-'d1], CC[BITWIDTH-'d2][x0], PP[BITWIDTH-'d2][x0]);
    end
    ADDER_FULL PP_row22(CC[BITWIDTH-'d3][BITWIDTH-'d1], ~IN[BITWIDTH-'d1][BITWIDTH-'d1],  CC[BITWIDTH-'d2][BITWIDTH-'d2],  CC[BITWIDTH-'d2][BITWIDTH-'d1],  PP[BITWIDTH-'d2][BITWIDTH-'d1]);
