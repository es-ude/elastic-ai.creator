...
from elasticai.creator.ir2vhdl import DataGraph
...
def build_declarations_for_combinatorial(impl: DataGraph) -> Iterator[str]:
...
def build_instantiations_for_combinatorial(impl: DataGraph) -> Iterator[str]:
    nodes = tuple(wrap_node(n) for n in impl.nodes.values())
