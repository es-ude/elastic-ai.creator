...
def test_network(
    ...
    build_design(registry["network"], registry, vhdl_dir, prepare_ir2vhdl_for_sim())

    with torch.no_grad():
        output_tensor = sw_model(input_tensor)

    _write_inputs_outputs_to_fixture(
        cocotb_test_fixture, output_tensor, input_bits, vhdl_dir
    )
...
def _write_inputs_outputs_to_fixture(
    cocotb_fixture: CocotbTestFixture,
    output_tensor: torch.Tensor,
    input_bits: list[int],
    build_dir: Path,
) -> None:
    output_bits = tensor_to_bit_vector(output_tensor)
    cocotb_fixture.write(
        {
