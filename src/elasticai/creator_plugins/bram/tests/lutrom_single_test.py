...
from elasticai.creator.arithmetic import int_converter, int_arithmetic
...
@cocotb.test()
@eai_testbench
async def lutrom_read(dut, bitwidth: int, num_params: int, is_signed: bool, ram: list) -> None:
    period_clk = 5
...
def test_lutrom_single_port_build(
    ...
    data = [randint(arith.minimum_as_integer, arith.maximum_as_integer) for _ in range(num_params)]
    data0 = data.copy()
    data0.reverse()
