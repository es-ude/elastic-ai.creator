...
cocotb_settings = dict(
    src_files=["heaviside.v"],
    path2src=Path(test_dut.__file__).parent / "verilog",
    top_module_name="ACT_HEAV",
    cocotb_test_module="elasticai.creator_plugins.act_func.tests.heaviside_tb",
    params={"BITWIDTH": 4},
)


def act_heaviside(a: int, bitwidth: int, fraction: int) -> int:
    range = Fxp(val=[0.0, +1.0], n_word=bitwidth, n_frac=fraction, signed=True)
...
async def act_access_positive(dut):
    ...
    assert dut.Q.value.to_signed() == act_heaviside(A, bitwidth, 2)
