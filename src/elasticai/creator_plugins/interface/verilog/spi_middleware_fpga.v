...
module SPI_MIDDLEWARE#(
    ...
    // --- DRDY posedge detection
    reg shift_drdy;
    wire spi_new_data;
    assign spi_new_data = !shift_drdy && SPI_DRDY;
    ...
    always@(posedge CLK_SYS) begin
        if(~RSTN) begin
            ...
            data_spi_tx_used <= (spi_new_data && register == REG_ECHO) ? {register, data} : data_spi_tx_used;
            LED <= (spi_new_data && register == REG_SYSTEM) ? ((data[1]) ? ~LED : data[0]) : LED;
        end
    end
endmodule