...
def test_build_test_linear_sigmoid(
    total_bits: int,
    frac_bits: int,
    features_in: int,
    features_out: int,
    ...
    file_name = (
        f"TestLinearSigmoid_{total_bits}_{frac_bits}_{features_in}x{features_out}"
    )
    ...
    dut = Sequential(
        ...
        nn_creator.Sigmoid(total_bits=total_bits, frac_bits=frac_bits, num_steps=16),
    )
    # --- Adapting values
