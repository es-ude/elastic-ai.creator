...
from elasticai.creator.nn.fixed_point import (Linear as LinearCreator,
...
def generate_torch_linear(settings: SettingsTest = RecommendedSettings) -> list:
    linear_layer = Linear(settings.size_input_layer, settings.size_output_layer, bias=True)
...
def generate_torch_batchlinear(settings: SettingsTest = RecommendedSettings) -> list:
    ...
    batch1d_layer.weight.data = tensor([.75])
...
def generate_creator_batchlinear(settings: SettingsTest = RecommendedSettings) -> list:
    ...
    batch1d_layer.bn_weight.data = tensor([.75])
...
def build_creator_sequential(nn_layer: list, do_training: bool=False) -> SequentialCreator:
...
class TestCreatorBatchTraining(TestCase):
    model_torch = build_torch_sequential(generate_torch_batchlinear(), do_training=False)
    model_creator = build_creator_sequential(generate_creator_batchlinear(), do_training=False)
    ...
    def test_data_output_size_torch(self):
        testing.assert_close(self.model_torch(self.data_in).size(1), RecommendedSettings.size_output_layer)

    def test_data_output_size_creator(self):
        testing.assert_close(self.model_creator(self.data_in).size(1), RecommendedSettings.size_output_layer)

    def test_creator_batch(self):
        data_out_torch = self.model_torch(self.data_in)
        data_out_creator = self.model_creator(self.data_in)
        testing.assert_close(data_out_torch, data_out_creator,
...
class TestCreatorBatchEvaluation(TestCase):
    ...
    model_creator = build_creator_sequential(generate_creator_batchlinear(), do_training=True)
    data_in = generate_test_data()
