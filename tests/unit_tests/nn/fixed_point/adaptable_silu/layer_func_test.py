...
def test_silu_compared_torch(total_bits: int, frac_bits: int) -> None:
    plot_results = True
    ...
    act1 = nn_creator.AdaptableSiLU(
        total_bits, frac_bits, num_steps=num_steps, sampling_intervall=range
    )
    out1 = act1(Tensor(stimulus))

    if plot_results:
        plt.title(f"Adaptable SiLU (n={total_bits}, {frac_bits})")
