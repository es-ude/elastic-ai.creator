...
def test_vhdl_code_matches_expected_pprelu() -> None:
...
begin
    ...
    pprelu_process : process(signed_x)
    begin
        if enable = '0' then
            signed_y <= to_signed(0, BITWIDTH_OUTPUT);
        else
            if signed_x <= to_signed(-128, BITWIDTH_OUTPUT) then signed_y <= to_signed(-42, BITWIDTH_OUTPUT);
...
""".splitlines()
    actfunc = nn_creator.PrecomputedPReLU(
        total_bits=8,
        frac_bits=5,
        num_steps=4,
    ...
    design = actfunc.create_design("pprelu")
    design.save_to(build_path)
    actual = cast(InMemoryFile, build_path["pprelu"]).text
    for text in actual:
        print(text)
