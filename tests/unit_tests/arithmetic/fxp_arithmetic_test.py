...
def test_cut_float_to_integer_signed(total_bits: int, frac_bits: int) -> None:
    config = FxpArithmetic(total_bits=total_bits, frac_bits=frac_bits, signed=True)
...
def test_cut_float_to_integer_unsigned(total_bits: int, frac_bits: int) -> None:
    config = FxpArithmetic(total_bits=total_bits, frac_bits=frac_bits, signed=False)
    chck = [
        config.minimum_as_integer,
        1,
        config.maximum_as_integer - 1,
        config.maximum_as_integer,
    ]
    stimuli = [
        config.minimum_as_rational,
        config.minimum_step_as_rational,
        config.maximum_as_rational - config.minimum_step_as_rational,
        config.maximum_as_rational,
    ]
    rslt = [config.cut_as_integer(val) for val in stimuli]
    assert rslt == chck
