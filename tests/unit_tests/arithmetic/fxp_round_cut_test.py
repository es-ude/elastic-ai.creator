...
from elasticai.creator.arithmetic.fxp_round_cut import (
    CutToFixedPoint,
    RoundToFixedPoint,
)
...
def round_to_fxp(inputs: list[float] | torch.Tensor) -> torch.Tensor:
    ...
    return cast(
        torch.Tensor,
        RoundToFixedPoint.apply(inputs, FxpArithmetic(total_bits=4, frac_bits=2)),
    )
...
def cut_to_fxp(inputs: list[float] | torch.Tensor) -> torch.Tensor:
    ...
    return cast(
        torch.Tensor,
        CutToFixedPoint.apply(inputs, FxpArithmetic(total_bits=4, frac_bits=2)),
    )
